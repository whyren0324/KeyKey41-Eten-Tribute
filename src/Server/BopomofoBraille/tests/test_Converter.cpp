...
static std::string convertBpmfToBraille(
    const std::string& input, BrailleType type = BrailleType::UNICODE) {
...
static std::vector<BopomofoBrailleConverter::Token> convertBrailleToTokens(
    const std::string& input, BrailleType type = BrailleType::UNICODE) {
...
TEST_SUITE("BopomofoBrailleConverter") {
  ...
  TEST_CASE("Test mix 1 - 2") {
    std::string input =
        "ㄒㄧㄠˇㄇㄞˋㄓㄨˋㄧ音";  // Wait, in the test it was "ㄓㄨˋㄧㄣ"
                                  // (Zhuyin) - ah, TS says ㄓㄨˋㄧㄣ but wait,
                                  // the TS code on line 191 is
                                  // ㄒㄧㄠˇㄇㄞˋㄓㄨˋㄧㄣ
  ...
  TEST_CASE("Edge cases and error handling") {
    ...
    SUBCASE("should handle space-only input") {
      ...
      std::string r1 = convertBpmfToBraille(input);
      std::string r2 = convertBrailleToBpmf(r1);
      CHECK(r2 == "   ");  // Wait, TS expectation: expect(r2.trim()).toBe("");
                           // we can check that it's whitespace
